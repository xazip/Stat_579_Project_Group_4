...
df_top5 %>% ggplot(aes(x = Month_to_Comptroller, fill = City_Department_Involved))+
  ...
  labs(title="Lawsuits by Month for the Five Most Frequently Sued Departments",
       x = "Month",
       y="Count")#+
    #facet_wrap(~Year_to_Comptroller)
```
...
Let's further investigate the city-wide lawsuits.
...
```
```{r, echo =T}
payments <- data.frame(payment_amount = c(12000000, 3600000+84150.40, 11848340, 3670092), payment_type = c("City Settlement", "Ordered Attorney Fees & Costs", "Total Payment to Payees", "Total Attorney Fees & Costs"))
payments$payment_type <- factor(payments$payment_type, levels=c("Ordered Attorney Fees & Costs", "Total Attorney Fees & Costs", "City Settlement", "Total Payment to Payees"))

payments %>% group_by(payment_type) %>% 
  ggplot(aes(x=payment_type, y = payment_amount)) +
  geom_col()+
...
df_cpd %>% ggplot(aes(x = Year_to_Comptroller))+
  geom_bar()+
