...
#-----------------------------------------------------------------------------------------------------------------------------------#
df %>% filter(City_Department_Involved == "POLICE") %>% {print(unique(.$Primary_Cause))}
df %>% filter(City_Department_Involved == "POLICE" & str_detect(Primary_Cause, "FALSE")) %>% {print(unique(.$Primary_Cause))}
df$Primary_Cause <- 
...
nrow(filter(df1, is.na(Administrative)))
...
Some quick graphs with the new columns made from `Case_Number`:
```{r}
df1 %>%
...
df1 %>%
  filter(Administrative, Total_Paid < 0) %>%
  arrange(Total_Paid)
# Hard to find online records of these
# People's Gas is a utilities company, so it would be believable that they could've
# damaged something while installing something and had to pay the City money

# noticed that many of the negative values have duplicate positive values
# check if all negative values are duplicates
df1 %>%
  filter(Administrative, Total_Paid < 0) %>%
  arrange(Year_to_Comptroller)

# definitely include this in report vvvvv
df1 %>%
  filter(!is.na(Court)) %>%
...
# maybe investigate "large" values separately?
# yes! see quantile
...
# Choose certain depts, choose primary cause(?)
...
nrow(fire_long)
nrow(filter(df1, City_Department_Involved == "FIRE DEPARTMENT"))
...
# https://www.wbez.org/stories/firefighters-settle-on-reverse-discrimination-case/d972bd91-012a-4fc7-87aa-8a7dbe764d04
...
df1 %>%
  ...
  filter(!is.na(Length), City_Department_Involved=="POLICE") %>%
  filter(Primary_Cause %in% c("FALSE ARREST", "EXCESSIVE FORCE", "PROPERTY DAMAGE/MVA",
...
large_dept %>%
  ...
  ylab("log10(Amount City Paid in Lawsuit)")
...
large_dept %>%
  filter(Disposition != "CONSULTANT", City_Department_Involved=="POLICE",
         !is.na(Year_Filed)) %>%
  group_by(Disposition, Year_Filed) %>%
  summarize(n=n()) %>%
  ggplot(aes(x = factor(Year_Filed), y = n, color = Disposition)) +
  geom_point() + 
  geom_line(aes(group=Disposition)) +
  ylab("Number of Cases") +
  xlab("Year Filed") +
...
large_dept %>%
  filter(Disposition != "CONSULTANT", !is.na(Year_Filed)) %>%
  group_by(Disposition, Year_Filed) %>%
  summarize(n=n()) %>%
  ggplot(aes(x = factor(Year_Filed), y = n, color = Disposition)) +
  geom_point() + 
